...
def _gen_detection_video(num_epoch,video_name, video_score, video_cls, video_info, opt, num_prop=200, topk=2):
    ...
    df = pd.read_csv(os.path.join(opt["output"], 'results_'+str(num_epoch) + "/v_" + video_name + ".csv"))
...
def gen_detection_multicore(opt,num_epoch):
    # get video duration
    ...
    detection = parallel(delayed(_gen_detection_video)(num_epoch,vid, cls_data_score[vid], video_cls, infer_dict['v_' + vid], opt)
...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        gen_detection_multicore(opt,num_epoch=i)
